...
public class RotaServiceImpl implements RotaService {
	...
	public String registrarRotas(String pXmlRotas)
		...
		if (pXmlRotas == null || pXmlRotas.isEmpty()) {
			throw new IllegalArgumentException(
			        "XML (pXmlRotas) não pode ser nulo ou vazio");
		}
		StringBuilder builder = new StringBuilder("<retorno>");
		Object objeto = new XmlUtil().adicionarAlias("rota", Rota.class)
	...
	public String registrarRota(String pOrigem, String pDestino,
	        BigDecimal pDistancia) throws IllegalArgumentException {
	...
	private void gravarRota(Rota pRota) throws IllegalArgumentException {
		if (pRota.isOrigemIgualDestino()) {
			throw new IllegalArgumentException(
			        "Uma Rota não deve ter a mesma origem e destino: "
			                + pRota.toString());
		}
		pRota.gerarSteps();
		Rota rotaBanco = repository.buscarPorSteps(pRota.getSteps());
		if (rotaBanco == null
		        || rotaBanco.getDistancia().compareTo(pRota.getDistancia()) < 0) {
			rotaBanco = pRota;
			...
			for (Rota rotaEncontrada : listaRotas) {
				...
				String step = rotaEncontrada.getSteps().replace(
				        rotaBanco.getOrigem(), rotaBanco.getSteps());
